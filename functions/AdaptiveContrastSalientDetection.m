...
if strcmp(ft_param,'fft')
    %低频
    l_fq=e_fq(1:11,1:11);    %目标最小尺寸是图像的1/20
    ...
    %低频
    l_fq=e_fq(1:21,1:21);          %目标最小尺寸是图像的1/20
    lpx=x(1:21);
    lpy=y(1:21);
...
if strcmp(ft_param,'fft')%结合二阶导数
%     lfx=filter2([0.5,0.5],lfx);%平滑滤波
    d2fx=[lfx(1)-lfx(2),2*lfx(2:10)-lfx(1:9)-lfx(3:11),0];
    lfx=lfx-d2fx;
    
%     lfy=filter2([0.5;0.5],lfy);%平滑滤波
    d2fy=[lfy(1)-lfy(2);2*lfy(2:10)-lfy(1:9)-lfy(3:11);0];
    lfy=lfy-d2fy;
else
